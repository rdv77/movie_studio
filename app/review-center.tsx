...
export function ReviewCenter({p,busy,submit,open}:{p:Project;busy:boolean;submit:(action:string,data:unknown)=>Promise<void>;open:(stage:number,itemId:string)=>void}){
  const [checked,setChecked]=useState<string[]>([]),[measuring,setMeasuring]=useState(false),[error,setError]=useState('');
  const rows=reviewRows(p),pending=rows.filter(r=>r.status!=='approved');
  ...
  return <details className="editor-surface p-5 mb-5" open={pending.some(r=>r.status==='conflict')}><summary><b>Проверка и быстрое утверждение · {pending.length} материалов</b></summary>
    <p className="muted">Здесь собраны все промежуточные этапы. Новые готовые варианты можно утвердить вместе. Материалы с изменённой основой отметьте после просмотра; конфликты сначала нужно исправить.</p>
    <div className="row wrap my-3"><Button variant="outline" disabled={busy||measuring} onClick={measure}>{measuring?'Проверяем файлы…':'Проверить длительность видео и речи'}</Button><Button variant="outline" disabled={busy} onClick={()=>setChecked(pending.filter(r=>r.status==='ready').map(r=>r.itemId))}>Выбрать все готовые</Button></div>
    ...
    <Button className="mt-4" disabled={busy||measuring||!checked.length} onClick={async()=>{await submit('approveReview',{selections:pending.filter(r=>checked.includes(r.itemId)&&r.variantId).map(r=>({itemId:r.itemId,variantId:r.variantId,reviewed:r.status==='review'}))});setChecked([]);}}>Утвердить отмеченные материалы ({checked.length})</Button>
    <p className="muted small">Изменившийся аниматик нужно собрать заново: утверждение карточек не меняет уже сохранённый видеофайл.</p>
  </details>;
}
