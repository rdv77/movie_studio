...
export const POST = api(async (req, ctx) => {
  ...
  for (const row of rows) if (!row.item) {
    ...
    row.item = { id: id(), stage: 6, title: row.title, sourceShot: { scriptId: row.scriptId, title: row.title, scriptVersion:row.scriptVersion,shotId:row.shotId,sceneId:row.sceneId }, variants: [] };
  ...
  const jobs: Job[] = s.plans.map(input => {
    ...
    if(row.timingIssue)throw Error(`${row.title}: ${row.timingIssue}`);
