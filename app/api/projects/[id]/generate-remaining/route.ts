...
import { videoCharacterRefs, videoCharacters, withCharacterIdentity, assertCharacterRefLimit } from '@/lib/characters';
...
export const POST = api(async (req, ctx) => {
  ...
  for (const row of s.plans) {
    ...
    const characterRefs=videoCharacterRefs(p,m.provider,characterIds);assertCharacterRefLimit(characterRefs,7);
    ...
    const prompt=videoGenerationPrompt(p,item,row.prompt,characterIds);
