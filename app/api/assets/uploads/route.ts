import { api, owner, runtime, loadProject } from '@/lib/server';
...
import { z } from 'zod';
const schema = z.object({ projectId: z.string().uuid(), name: z.string().trim().min(1).max(160),
...
export const POST = api(async req => {
  ...
  await loadProject(user, data.projectId);
  ...
  try {
    await runtime.DB.prepare('INSERT INTO asset_uploads (id,owner,name,mime,size,upload_id,created,project_id) VALUES (?,?,?,?,?,?,?,?)')
      .bind(key, user, data.name, data.mime, data.size, upload.uploadId, now(), data.projectId).run();
