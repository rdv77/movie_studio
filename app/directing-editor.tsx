...
export function DirectingEditor({p,stage,busy,submit,open}:Props){
  ...
  return <section className="editor-surface p-5 mb-6 space-y-5" aria-label="Команда сценаристов и режиссёров">
    ...
    {!!d?.issues.length&&<div className="space-y-3"><h3>Проверка редактора</h3>
      <p className="muted">Редактор предлагает готовые изменения. Применение решений не запускает платную генерацию. После правок повторите проверку фильма и утвердите обновлённые планы.</p>
      <div className="row wrap"><Button variant="outline" disabled={locked} onClick={()=>generate('editor')}>Предложить решения / проверить заново</Button><Button disabled={locked||!d.patches.some(p=>!p.applied)} onClick={()=>call('applyAllSolutions')}>Применить все предложенные правки</Button></div>
