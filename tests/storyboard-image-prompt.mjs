...
const falModel='fal-qwen-image-edit-2511',falPrompt=P.storyboardImageRequest(p,frame,task,refs,1,1,falModel);
assert(falPrompt.length<=5000);assert(falPrompt.prompt.includes('ДЕЙСТВИЕ_0'));assert(falPrompt.prompt.includes('Петя'));assert(falPrompt.prompt.includes('УТВЕРЖДЁННЫЙ_СТИЛЬ'));
globalThis.state=structuredClone(p);response=await G.POST(req({...single,models:[falModel],estimates:{[falModel]:'300000000'}}),ctx);
assert.equal(response.status,200,await response.clone().text());assert.equal(state.jobs.at(-1).prompt,falPrompt.prompt);assert.deepEqual(state.jobs.at(-1).refs,refs);assert.equal(state.jobs.at(-1).estimate,'300000000');assert.equal(state.jobs.at(-1).actual,null);
globalThis.state=structuredClone(p);response=await B.POST(req({...batch,model:falModel,estimate:'300000000'}),ctx);assert.equal(response.status,200,await response.clone().text());assert.equal(state.jobs.length,p.jobs.length+11);assert.equal(state.jobs[p.jobs.length].prompt,falPrompt.prompt);assert.deepEqual(state.items,p.items);
...
console.log(`PASS compact storyboard prompts: old ${D.promptFor(p,frame,task,D.chosen(frame)).length} chars -> ${request.length}; 11 queued frames, approved profiles/style/ref order, identical preview/single/batch, stable saved tasks, 32000/32001 boundaries and atomic rejection. Fal single/batch, immutable approvals, cost reservation and idempotency. No paid calls.`);
