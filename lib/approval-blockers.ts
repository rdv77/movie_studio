  ...
  for (const item of p.items.filter(i => i.stage < stage && participates(p, i))) {
    ...
    if (!approved) {
      ...
      if (selected && selected.id !== approved.id && selected.deps === current) {
        ...
        reason += item.stage === 7 ? ' Откройте карточку, выберите подходящий ролик и нажмите «Утвердить этот ролик для текущей версии».' : item.stage===6 ? ' Откройте карточку, прослушайте запись и нажмите «Утвердить эту запись для текущей версии», если она подходит.' : item.stage===2 ? ' Откройте стиль, проверьте его и нажмите «Утвердить стиль для текущей версии».' : ' Откройте карточку и проверьте вариант. Если файл подходит, через «Правки» сохраните актуальную версию и утвердите её.';
      }
    }
